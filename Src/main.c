...
// Si el usuario envía 'F', responder con 'A'
if (rx_data == 'F')
{
reply = 'A';
}
...
char *welcome_msg = "Sistema iniciado correctamente.\r\n";
...
while (1)
{
  ...
  if (HAL_GetTick() - last_heartbeat_time >= 5000) // Cada 5000 ms (5 segundos)
  {
  ...
  if (button_flag == 1)
  {
    ...
    // 2. Calcular cuándo apagarlo (7 segundos desde ahora)
    led_ext_off_time = HAL_GetTick() + 7000;

    // 3. Enviar mensaje por UART
    sprintf(tx_buffer, "Temporizador activado. LED encendido.\r\n");
  ...
  if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
  {
    ...
    sprintf(tx_buffer, "Temporizador completado. LED apagado.\r\n");
    HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
